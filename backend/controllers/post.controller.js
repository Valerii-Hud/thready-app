...
export const createPost = async (req, res) => {
  try {
    ...
    if (!text && !img) {
      return res.status(400).json({ error: 'Post must have tet or image' });
    }

    if (img) {
      const uploadedResponse = await cloudinary.uploader.upload(img);
      img = uploadedResponse.secure;
    }
...
export const deletePost = async (req, res) => {
  try {
    ...
    const isAdmin = process.env.ADMIN_ACCOUNT === currentUser;
    ...
    if (post.user.toString() !== currentUser && !isAdmin) {
      return res
        .status(401)
